...
*   **Reference-Based Context:** When dealing with external artifacts (files, messages), clients and adapters provide context by including [`ArtifactReference`](../../../Nucleus.Abstractions/Models/ArtifactReference.cs) objects within their API requests (typically in an [`AdapterRequest`](../../../Nucleus.Abstractions/Models/AdapterRequest.cs) payload). **Clients do not typically send raw file content directly to the top-level API endpoints if a reference mechanism is more appropriate.**
